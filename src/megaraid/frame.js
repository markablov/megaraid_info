...
const FrameHeader = E.types.FrameHeader = ref_struct({
  /* 0x00 */ cmd: uint8,
...
const PDSCSIIOFrame = E.types.PDSCSIIOFrame = ref_struct({
  /* 0x00 */ hdr: FrameHeader,
  /* 0x18 */ sense_buf_phys_addr_lo: uint32,
  /* 0x1C */ sense_buf_phys_addr_hi: uint32,
  /* 0x20 */ cdb: ref_buffer_type(16),
  /* 0x30 */
...
const fill_frame_header = (hdr, cmd, target, inSize, outSize) => {
  hdr.cmd = cmd;
  hdr.sense_len = 0;
  hdr.cmd_status = 0xFF;
  hdr.scsi_status = 0;
...
E.build_dcmd_frame = (opcode, outSize, mbox, inSize = 0) => {
  let frame = new DCMDFrame();
  // XXX: need to support multiply devices through one controller
  fill_frame_header(frame.hdr, E.const.FRAME_CMD.MFI_CMD_DCMD, 0, inSize, outSize);
  frame.opcode = opcode;
  if (mbox)
    frame.mbox = mbox;
  return frame;
